  ...
  <q-page padding>
    <q-card>
      ...
      <template v-slot:body="props">
          <q-tr :props="props">
            <q-td key="namauang" :props="props">{{ props.row.namauang }}</q-td>
            <q-td key="semesteruang" :props="props">{{ props.row.semesteruang }}</q-td>
            <q-td key="sppuang" :props="props">{{ props.row.sppuang }}</q-td>
      ...
      <q-dialog v-model="openedit" v-if="openedit">
        <q-card style="width: 700px; max-width: 80vw;">
          ...
          <q-card-section style="max-height: 50vh" class="scroll">
            <q-input label="namauang" v-model="activedata.namauang"></q-input>
            <q-input label="semesteruang" v-model="activedata.semesteruang"></q-input>
            <q-input label="sppuang" v-model="activedata.sppuang"></q-input>
          </q-card-section>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'semesteruang', align: 'center', label: 'Semester', field: 'semesteruang', sortable: true },
        { name: 'sppuang', align: 'center', label: 'Nominal SPP', field: 'sppuang', sortable: true },
  ...
  methods: {
    getData () {
      this.$axios.get('/keuangan/getalluang')
        .then(res => {
          console.log(res)
    ...
    edit () {
      this.$axios.put('keuangan/updateuang/' + this.activedata._id, this.activedata)
        .then(res => {
          if (res.data.error) {
    ...
    confirm (id) {
      this.$q.dialog({
        ...
        this.$axios.delete('/keuangan/deleteuang/' + id)
          .then(res => {
            if (res.data.error) {
              this.showNotif(res.data.pesan, 'negative')
            } else {
