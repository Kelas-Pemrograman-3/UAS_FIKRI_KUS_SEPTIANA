  ...
  <q-page padding>
    <q-card>
      ...
      <template v-slot:body="props">
          <q-tr :props="props">
            <q-td key="nidn" :props="props">{{ props.row.nidn }}</q-td>
            <q-td key="password" :props="props">{{ props.row.password }}</q-td>
            <q-td key="namadosen" :props="props">{{ props.row.namadosen }}</q-td>
            ...
            <q-td key="dosen" :props="props">
              <span v-for="(data, i) in props.row.dosen" :key="i">{{data.nama}}</span>
            </q-td>
      ...
      <q-dialog v-model="openedit" v-if="openedit">
        <q-card style="width: 700px; max-width: 80vw;">
          ...
          <q-card-section style="max-height: 50vh" class="scroll">
            <q-input label="NIDN" v-model="activedata.nidn"></q-input>
            <q-input label="Password" v-model="activedata.password"></q-input>
            <q-input label="Lecturer" v-model="activedata.namadosen"></q-input>
            <q-input label="Birth" v-model="activedata.tgllahir"></q-input>
            <q-input label="Gender" v-model="activedata.jeniskelamin"></q-input>
            <q-input label="Phone Number" v-model="activedata.notelepon"></q-input>
          </q-card-section>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'namadosen', align: 'center', label: 'Lecturer', field: 'namadosen', sortable: true },
        { name: 'tgllahir', align: 'center', label: 'Birthdate', field: 'tgllahir', sortable: true },
  ...
  methods: {
    getData () {
      this.$axios.get('/dosen/getalldosen')
        .then(res => {
          console.log(res)
    ...
    edit () {
      this.$axios.put('dosen/updatedosen/' + this.activedata._id, this.activedata)
        .then(res => {
          if (res.data.error) {
    ...
    confirm (id) {
      this.$q.dialog({
        ...
        this.$axios.delete('/dosen/deletedosen/' + id)
          .then(res => {
            if (res.data.error) {
              this.showNotif(res.data.pesan, 'negative')
            } else {
