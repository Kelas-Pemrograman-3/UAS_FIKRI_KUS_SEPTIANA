      ...
      <q-dialog v-model="openedit" v-if="openedit">
        <q-card style="width: 700px; max-width: 80vw;">
          ...
          <q-card-section style="max-height: 50vh" class="scroll">
            <q-input label="NPM" v-model="activedata.npm"></q-input>
...
export default {
  ...
  methods: {
    getData () {
      this.$axios.get('/mahasiswa/getallmhs')
        .then(res => {
          console.log(res)
    ...
    edit () {
      this.$axios.put('mahasiswa/updatemhs/' + this.activedata._id, this.activedata)
        .then(res => {
          if (res.data.error) {
    ...
    confirm (id) {
      this.$q.dialog({
        ...
        this.$axios.delete('/mahasiswa/deletemhs/' + id)
          .then(res => {
            if (res.data.error) {
              this.showNotif(res.data.pesan, 'negative')
            } else {
